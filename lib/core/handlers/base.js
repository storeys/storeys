define(
    ...
    function(require, module, settings, urlresolver, binds, urllib) {
      ...
      /**
       * Normalize `path` in the context object.
       */
      function normalize_path(context) {
        var base    = require.toUrl(''),
            baseurl = urllib.parse(base),
            url     = urllib.parse(base + context.path);
      ...
      //                  Public
      // ===========================================
      function get_response(context, cb) {
        verbose && console.log(LOG_PREFIX + 'get_response(' + context.path + ') method: ' + context.method);

        var request;

        context = normalize_path(context);

        dispatch_middleware(context, function(response) {
          if (typeof response !== 'undefined') {
            verbose && console.log(LOG_PREFIX + 'middleware returns a response(' + context.path + ')');
            cb(response);
          } else {
            resolve(context.path, function(params, view) {
              ...
              if (found) {
                ...
                verbose && console.log(LOG_PREFIX + 'found no match for url: ' + context.path);
              }
            });
          }
        });
